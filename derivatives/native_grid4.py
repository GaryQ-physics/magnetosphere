...
for vvar in ['j','b','b1']:
    print('computing partials for '+vvar)

    partials = np.zeros((nBlock, nI, nI, nI, 3,3), dtype=np.float32)
    for i in range(nI):
        for j in range(nJ):
            for k in range(nK):
                if i == 0 or i == nI-1:
    ...
    print('starting derivatives for '+vvar)
...
jRpartials = np.zeros((nBlock, nI, nI, nI, 3,3), dtype=np.float32)
for i in range(nI):
    for j in range(nJ):
        for k in range(nK):
...
direct = conf[run+'_derived']  \
        + 'derivatives/%.2d%.2d%.2dT%.2d%.2d%.2d/native_grid/'%util.tpad(time, length=6)

if not os.path.exists(direct): os.makedirs(direct)
df_ends_as_nan.to_pickle(direct+'df_ends_as_nan.pkl')
print('saved ' + direct+'df_ends_as_nan.pkl')
