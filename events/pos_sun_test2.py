...
filename = conf['run_path'] + '3d__var_3_e' + str(year) + str(month) + str(day) + '-070000-000.out.cdf'

from datetime import datetime
epoch = datetime(2000, 1, 1, 12, 0, 0)
delta = datetime(year, month, day, int(hours), int(minutes), int(seconds)) - epoch
...
for i in range(n):
    ...
    GSM_spacepy = ps.MAGtoGSM([MAG[i,0], MAG[i,0], MAG[i,2]], time, 'car', 'car')
