...
def traceFile(filename, IC, method='vtk', debug=True):
    ext = filename[-4:]
    ...
    if method == 'scipy'
        if ext == '.out':
            ...
            return trace(X, B, IC, method=method, debug=debug)
        elif ext == '.cdf':
            pass
        else:
            raise ValueError
        ...
        if ext == '.vtk':
            ...
            reader.Update()
            trace(None, reader, IC, method=method, debug=debug)
        elif ext == '.out':
            pass # how
        elif ext == '.cdf':
            pass # how
...
def trace(Space, Field, IC, method='scipy', debug=True):
    ...
    if method=='scipy':
        ...
        if isinstance(Field, types.FunctionType):
            ...
            return trace(None, Fcallable, IC, method=method, debug=debug)


    elif method=='vtk':
        assert(isinstance(Field,vtk.vtkObject)
        print(Field)
        print(type(Field))
...
if __name__=='__main__':
    ...
    ret = trace(None, '/tmp/tmp.vtk', IC, method='vtk')
