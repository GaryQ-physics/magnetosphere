...
def dB_from_CDF(run, time, mlat, mlon, var='dB', para=True, debug=False,
    ...
    bsk.integrate(time, mlat, mlon, filename = cdf_fname,
...
def dB_from_CDF(run, time, mlat, mlon, var='dB', para=True, debug=False,
    ...
    if os.path.exists(out_filename):
        ...
        return [Xgrid, np.nan*np.empty((Nx*Ny*Nz, 3)), (Nx,Ny,Nz), None, out_filename]
    else:
        util.dlfile(cdf_fname, debug=True)
        bsk.integrate(time, mlat, mlon, filename = cdf_fname,
            para=para, xlims=xlims, ylims=ylims, zlims=zlims, d=d,
    ...
    return [Xgrid, dB, (Nx,Ny,Nz), texture, out_filename]
'''

'''
def cdf_to_data(run, time, mlat, mlon, var, para=True, debug=False,
       xlims=(-56., 8.), ylims=(-32., 32.), zlims=(-32., 32.), d=0.125):
